...
class MicrophoneSession(BaseSession):
    ...
    async def initialize(self) -> None:
        ...
        try:
            log.info(
                f"Starting initialization for microphone session {self.session_id}"
            )
            ...
            log.info(
                f"Microphone session {self.session_id} initialized, publishing to {self.audio_subject}"
            )
    ...
    async def cleanup(self) -> None:
        ...
        if self.streaming_task:
            try:
                ...
                log.warning(
                    f"Streaming task for session {self.session_id} did not stop gracefully"
                )
        ...
        while not self._stop_event.is_set():
            try:
                ...
                ack = await self.js.publish(
                    self.audio_subject, chunk_msg.SerializeToString()
                )
        ...
        try:
            ...
            try:
                ...
                if "stream name already in use" in str(e).lower():
                    ...
                    log.info(
                        f"JetStream stream 'AUDIO' already exists with {stream.state.messages} messages"
                    )
                else:
                    raise
